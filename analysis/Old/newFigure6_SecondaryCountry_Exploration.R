...
a <- ggplot(subset(bp_df_long_mean, metric == "Daily Incidence"), 
  ...
  guides(col = guide_legend(title = "R0"),
         fill = "none") +
  theme_bw() +
  labs("Hospitalisation Detection Threshold", y = "Time to Detection (Days)") +
...
cowplot::plot_grid(a, b, nrow = 1, rel_widths = c(1, 2),
                   labels = c("A", "B"))
...
if (fresh_run) {
  ...
  saveRDS(model_outputs, "outputs/Figure5_SecondaryCountry_Exploration/Figure5_PrimarySecondary_TimingComparison_DifferentNPIs_CalendarDays.rds")
} else {
  model_outputs <- readRDS("outputs/Figure5_SecondaryCountry_Exploration/Figure5_PrimarySecondary_TimingComparison_DifferentNPIs_CalendarDays.rds")
}
...
deaths_averted <- ggplot(subset(model_outputs2, NPI_int != 9 & 
  ...
  facet_grid(NPI_scenario ~ specific_vaccine_start,
             labeller = as_labeller(c(`100`='Specific Vaccine In 100 Days', 
                                      `220`='Specific Vaccine In 220 Days',
...
ggsave(filename = "figures/Figure_5_SecondaryCountry_Exploration/Figure5_Secondary_TimingComparison_DifferentNPIs_CalendarDays.pdf",
       plot = deaths_averted,
       width = 9.5,
       height = 6)

secondary_legend <- cowplot::plot_grid(deaths_averted + theme(legend.position = "right"))
ggsave(filename = "figures/Figure_5_SecondaryCountry_Exploration/Figure5_legend.pdf",
