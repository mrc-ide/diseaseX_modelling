...
## Ask Greg why vaccine_booster_initial_coverage is in as a parameter in the general running functions,
## but not in https://github.com/mrc-ide/squire.page/blob/main/inst/odin/nimue_booster.R 
...
hosp_bed_capacity <- 100000000                                           # hospital bed capacity
ICU_bed_capacity <- 100000000                                            # ICU bed capacity

### Disease Severity Parameters (NEED TO CHECK THESE AND TALK TO AZRA ABOUT THEM)
ifr_scaling <- 1                                                         # Azra's IFR scaling thing (need to check, especially given generated by hand in spreadsheet and country-specific contact matrices being used)
...
######  NOTE - IF THERE'S NO BOOSTER PRIORITISATION, HOW ARE WE ENSURING THE ELDERLY GET PRIORITISED FOR VACCINATION HERE????
######         I DON'T THINK WE ARE???
######         NEED TO DOUBLE CHECK THIS - NO-ONE UNDER PRIORITY AGE GROUPS SHOULD BE BEING VACCINATED (AS DISEASE X FOR THEM IS PRIMARY VACCINE SERIES.)
######         PROBLEM: NON-PRIORITY GROUPS ARE BEING BOOSTED, AND ISSUE IS THAT time_to_coverage_v2_elderly IS CALCULATED ASSUMING THEY GET PRIORITISED FOR BOOSTERS
######         BUT WE DON'T HAVE BOOSTER PRIORITISATION IN CURRENTLY. AS A RESULT, BOOSTERS ARE DISTRIBUTED ACROSS ALL AGE-GROUPS,
######         AND WE DON'T HIT THE LEVELS OF COVERAGE IN ELDERLY WE SHOULD DO BY THE TIME time_to_coverage_v2_elderly IS UP
...
# though I suppose that's identical to what's being done for the boradly neutralising one currnetly
# BUT we probably want to start in oldest and work our way downwards, which Azra's code 
# doesn't have in place yet...

# note that as far as I can make out, when calculating how to distribute boosters, nimue will aim to get
# folks their first booster first, and then if there's no one eligible, will give others a second booster 
# check this with Greg. 
# Do we currently have vaccine dose distribution optimisation in place for boosters???
## I don't think so, as see here: https://github.com/mrc-ide/squire.page/blob/073ec7d107419d95aabfaba3735ffdc2609195a4/inst/odin/nimue_booster.R#L23-L26
## primary_first[i] has an age-index where as booster_first doesn't. 
## We get round it in the case here by not having boosters for <60s and only boosting oldest age-groups (where booster for them is diseaseX specific vaccine)

dim(r1$output)

output <- nimue::format(r1, compartments = c("S", "E", "IMild", "ICase", "IICU", "IHospital", "IRec", "R", "D"),
...
o1a <- nimue::format(r1, compartments = c("vaccinated_second_dose", "vaccinated_booster_dose"), 
                     reduce_age = FALSE) %>%
  filter(t > 1,
...
output2 <- rbind(output, o1a)


# plot(output2$t[output2$compartment == "S"], output2$value[output2$compartment == "S"])
# plot(output2$value[output2$compartment == "V2"], ylim = c(0, 10^6))
...
# note when vaccine 1 starts at t=5 and 2 starts at t=10, 
# you get far lower vaccine 2 coverage in elderly
# ggplot(data = o1ap, aes(x = t, y = value, col = compartment)) +
#   geom_line() +
#   facet_wrap(~age_group)
df <- data.frame(x1 = tt_primary_doses,
                 x2 = tt_primary_doses,
                 y1 = 0,
                 y2 = 1)
df2 <- data.frame(x1 = tt_booster_doses,
                  x2 = tt_booster_doses,
                  y1 = 0,
                  y2 = 1)
ggplot() +
...
check <- nimue::format(r1, compartments = c("first_doses_given", "second_doses_given", "booster_doses_given"), 
                       reduce_age = FALSE) %>%
  filter(t > 1,
         compartment == "first_doses_given" | compartment == "second_doses_given" | 
           compartment == "booster_doses_given") %>%
  group_by(replicate, t, age_group) 
ggplot() +
  geom_line(data = check, aes(x = t, y = value, col = compartment)) +
  facet_wrap(~age_group) +
  lims(x = c(0, 1000)) 
...
pop_df <- data.frame(age_group = sort(unique(o1a$age_group)),
                     population = standard_pop)
...
#   geom_line() +
#   facet_wrap(~age_group)
ggplot(data = o1ap, aes(x = t, y = prop, col = compartment)) +
  geom_line() +
