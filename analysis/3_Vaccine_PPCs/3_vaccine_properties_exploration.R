...
hosp_bed_capacity <- 1000000                                             # hospital bed capacity
ICU_bed_capacity <- 1000000                                              # ICU bed capacity
...
duration_R <- 10 * 365                     # duration of infection-induced immunity - assumed large as not considering waning (currently)
duration_V <- 10 * 365                     # duration of vaccine-induced immunity for both vaccines - assumed large as not considering waning (currently) - also need to make vaccine specific
dur_vacc_delay <- 14                       # mean duration from vaccination to protection
vaccine_1_start <- 5                       # time when BNPC starts being distributed
vaccine_2_start <- 10                      # note that this needs to be after v1 has been completed - WHY - NEED TO CHECK THIS???
coverage <- 0.95                            # proportion of the population vaccinated
...
lower_priority <- 14                       # index of the youngest age group given priority w.r.t vaccines (14 = 60+)
...
v1_daily_doses <- vaccination_rate * sum(pop) / 7  # what about the coverage stuff (i.e. does not assume coverage = 1, rather than e.g. 0.8)
v2_daily_doses <- vaccination_rate * sum(pop) / 7  # what about the coverage stuff (i.e. does not assume coverage = 1, rather than e.g. 0.8)
...
# Assumes no synergy between BNPSC and diseaseX-specific vaccine
...
tt_primary_doses <- c(0, vaccine_1_start, round(vaccine_1_start + time_to_coverage_v1), vaccine_2_start + time_to_coverage_v2_elderly) 
...
##     the elderly, for whom "primary" vaccine is BNPSC).
...
r1 <- run_booster( 
  ...
  population = pop,                                                          # population to be simulated
