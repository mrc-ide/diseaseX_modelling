...
quarantine_time <- function(n) { # from https://pmc.ncbi.nlm.nih.gov/articles/PMC7511527/?utm_source=chatgpt.com#cesec10
  days <- 1:5
  p <- c(0.20, 0.20, 0.20, 0.20, 0.20)
  sampled_quarantine_times <- sample(days, size = n, replace = TRUE, prob = p)
  return(sampled_quarantine_times)
} 
prob_quarantine_contact_traced <- 0.47 # from the same article - ~53% traced (via app-like) * 90% adhering
prob_quarantine_symptoms <- 0.9        # 90% adherence
quarantine_efficacy_scan <- c(0, 0.35, 0.65) # from https://pmc.ncbi.nlm.nih.gov/articles/PMC7511527/?utm_source=chatgpt.com#cesec10
...
#### NEED TO ADD IN A SCENARIO WITH NO QUARANTINING OR ISOLATION
...
if (fresh_run_R0_sensitivity_analysis) {
  ...
  SC1_storage_nothing <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_2weeks <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_1week <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_2days <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_instant <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  
  SC2_storage_nothing <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_2weeks <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_1week <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_2days <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_instant <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  
  SC1_storage_nothing_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_2weeks_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_1week_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_2days_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC1_storage_vacc_instant_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  
  SC2_storage_nothing_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_2weeks_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_1week_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_2days_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  SC2_storage_vacc_instant_nth_day <- array(data = NA, dim = c(iterations, length(R0_scan), length(quarantine_efficacy_scan)))
  ...
  df_SC1_nothing <- convert_array(arr = SC1_storage_nothing, scenario = "no_vaccination", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_nothing <- convert_array(SC2_storage_nothing, scenario = "no_vaccination", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_2weeks <- convert_array(SC1_storage_vacc_2weeks, scenario = "2weeks_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_2weeks <- convert_array(SC2_storage_vacc_2weeks, scenario = "2weeks_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_1week <- convert_array(SC1_storage_vacc_1week, scenario = "1week_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_1week <- convert_array(SC2_storage_vacc_1week, scenario = "1week_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_2days <- convert_array(SC1_storage_vacc_2days, scenario = "2days_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_2days <- convert_array(SC2_storage_vacc_2days, scenario = "2days_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_instant <- convert_array(SC1_storage_vacc_instant, scenario = "no_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_instant <- convert_array(SC2_storage_vacc_instant, scenario = "no_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  ...
  # Convert each array to a long data frame - epidemic size
  df_SC1_nothing_time_to_n <- convert_array_time_to_n(arr = SC1_storage_nothing_nth_day, scenario = "no_vaccination", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_nothing_time_to_n <- convert_array_time_to_n(SC2_storage_nothing_nth_day, scenario = "no_vaccination", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_2weeks_time_to_n <- convert_array_time_to_n(SC1_storage_vacc_2weeks_nth_day, scenario = "2weeks_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_2weeks_time_to_n <- convert_array_time_to_n(SC2_storage_vacc_2weeks_nth_day, scenario = "2weeks_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_1week_time_to_n <- convert_array_time_to_n(SC1_storage_vacc_1week_nth_day, scenario = "1week_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_1week_time_to_n <- convert_array_time_to_n(SC2_storage_vacc_1week_nth_day, scenario = "1week_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_2days_time_to_n <- convert_array_time_to_n(SC1_storage_vacc_2days_nth_day, scenario = "2days_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_2days_time_to_n <- convert_array_time_to_n(SC2_storage_vacc_2days_nth_day, scenario = "2days_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  df_SC1_instant_time_to_n <- convert_array_time_to_n(SC1_storage_vacc_instant_nth_day, scenario = "no_delay", pathogen = "SARS-CoV-1", R0_scan, quarantine_efficacy_scan)
  df_SC2_instant_time_to_n <- convert_array_time_to_n(SC2_storage_vacc_instant_nth_day, scenario = "no_delay", pathogen = "SARS-CoV-2", R0_scan, quarantine_efficacy_scan)
  
  # Combine all results into a single data frame.
  overall_bp_df_time_to_n <- bind_rows(
  ...
  overall_bd_df <- overall_bp_df %>%
    left_join(overall_bp_df_time_to_n, by = c("iteration", "scenario", "pathogen", "R0", "quarantine_efficacy"))
...
time_to_n_plot <- ggplot(time_to_n_df, 
...
if (fresh_run_vaccination_heatmaps) {
    ...
    for (j in 1:length(Tg_ratio_seq)) {
      for (k in 1:length(quarantine_efficacy_scan)) {
        for (l in 1:iterations) {
          ...
          quarantine_time_closure <- function(Tg_ratio) {
            function(n) {
              days <- Tg_ratio * 1:5
              p <- c(0.20, 0.20, 0.20, 0.20, 0.20)
              sampled_quarantine_times <- sample(days, size = n, replace = TRUE, prob = p)
          ...
          quarantine_time <- quarantine_time_closure(Tg_ratio_seq[j])
  ...
  quarantine_time <- quarantine_time_closure(Tg_ratio_fixed)
