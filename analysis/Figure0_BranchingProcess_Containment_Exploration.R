...
if (fresh_run_vaccination_heatmaps) {
  ...
  # vaccine_efficacy_seq <- seq(0.2, 1, 0.1)
  ...
  # 
  # ## Sensitivity Analysis - R0 vs Asymptomatic Transmission Proportion 
  # proportion_presymptomatic_seq <- seq(0.1, 0.8, 0.1)
  ...
  for (i in 1:length(R0_seq)) {
    for (j in 1:length(Tg_seq)) {
      for (k in 1:iterations) {
        generation_time <- function(n) { rgamma(n, shape = Tg_seq[i] * 2, rate = 2) } 
        infection_to_onset <- function(n) { rgamma(n, shape = (Tg_seq[i] * 2)/3, rate = 2) } # keeping proportion of presymptomatic transmission constant as Tg varies
        bp_out <- chain_sim_susc_ring_vacc2(offspring = "pois",
                                            mn_offspring = R0_seq[i],
                                            generation_time = generation_time,
                                            t0 = 0, tf = Inf, pop = pop, 
  ...
  Tg_ratio_seq <- c(0.25, 0.33, 0.5, 1, 2, 4, 5)
  ...
  for (i in 1:length(R0_seq)) {
    for (j in 1:length(Tg_ratio_seq)) {
      for (k in 1:iterations) {
        generation_time <- function(n) { rgamma(n, shape = 2 * vaccine_protection_delay * Tg_ratio_seq[i], rate = 2) } 
  ...
  storage_R0_TgRatio_df$iteration <- rep(1:iterations, each = length(R0_seq) * length(Tg_seq))
  ...
  storage_R0_Tg_df <- readRDS("outputs/branching_process_outputs/branchingProcess_R0_Tg_scan.rds")
...
ggsave(filename = "figures/Figure_1_BranchingProcess/Figure_1C_VaccineEfficacy_Sensitivity_Analysis.pdf", 
       plot = R0_vaccine_efficacy_plot, width = 2.4, height = 2.4)
...
R0_Tg_plot <- ggplot(storage_R0_Tg_df, aes(x = R0, y = Tg, fill = proportion_contained)) +
  geom_tile(colour = "black") +
  scale_fill_viridis_c(option = "mako", limits = c(0, 1), name = "Proportion\nContained",
                       direction = -1) +
  labs(x = "R0",
       y = "Mean Generation Time (Days)") +
  theme(axis.text = element_text(angle = 0),
        plot.title = element_text(hjust = 0.5, size = 20, face = "bold"),
        legend.title = element_text(size = 12),
        legend.text = element_text(size = 12),
        ...
        panel.border = element_rect(linetype = "solid", fill = NA, linewidth = 0.5)) +  # Add black border
  coord_cartesian(expand = FALSE)
ggsave(file = "figures/Figure_1_BranchingProcess/Figure_1F_Tg_Sensitivity_Analysis.pdf", object = R0_Tg_plot, width = 2.4, height = 2.4)

legend <- R0_preSymp_plot + theme(legend.position = "bottom")
ggsave(file = "figures/Figure_1_BranchingProcess/vaccination_heatmap_legend.pdf", plot = legend, width = 3, height = 3)
