...
## New version where stockpile size for BPSV is varied
vaccine_milestone_timingsNew <- function(country, 
                                         population_size, 
                                         detection_time, 
                                         bpsv_start, 
                                         specific_vaccine_start,
  ...
  elderly_pop_to_vaccinate_bpsv <- ceiling(sum(standard_pop[priority_age_groups]) * bpsv_stockpile_coverage) # 60+s receive primary (BNPCV) and booster (diseaseX-specific); under 60s receive just primary (diseaseX-specific)
  time_to_coverage_bpsv <- ceiling(elderly_pop_to_vaccinate_bpsv/daily_doses) + 1 ## might need to remove the +1s, TBD
  ...
  time_to_coverage_spec <- ceiling(elderly_pop_to_vaccinate_spec/daily_doses) + 1 ## might need to remove the +1s, TBD
...
create_vaccination_dose_seriesNew <- function(country, 
                                              population_size, 
                                              detection_time, 
                                              vaccine_scenario,
                                              bpsv_start, 
  ...
  if (vaccine_scenario == "specific_only") {
    ...
    ## initial campaign and so it doesn't make much difference. Just need to watch out
    ## for when detection_time+bpsv_start+bpsv_protection_delay+time_to_coverage_bpsv+1+4 is very
    ## close to the detection_time + specific_vaccine_start + spec_protection_delay time - EXACTLY
    second_doses[(detection_time+bpsv_start+bpsv_protection_delay+time_to_coverage_bpsv+1+1)] <- daily_doses_bpsv
    second_doses[(detection_time+bpsv_start+bpsv_protection_delay+time_to_coverage_bpsv+1+2)] <- daily_doses_bpsv
    second_doses[(detection_time+bpsv_start+bpsv_protection_delay+time_to_coverage_bpsv+1+3)] <- daily_doses_bpsv
    # CHECK THIS DOESN'T FUCK SHIT UP WITH THE NEW VERSION
