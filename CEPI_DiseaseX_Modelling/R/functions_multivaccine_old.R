...
run_scenario <- function(target_pop = 1e6,
  ...
  #generate vaccine coverage matrix - rows are v1 and v2 for the booster model
...
format_out <- function(out, scenarios){
  ...
  outcf <- filter(out1, vaccine_2_start == runtime) %>%
      select(-vaccine_1_start, -vaccine_2_start,-timing2) %>%
 ...
 # out2 <- filter(out1, vaccine_start < 365)
  summaries <- left_join(out1, outcf,by=c("target_pop","income_group","R0","Rt1","Rt2","timing1","ifr_scaling","coverage", 
                                          ...
                                          "lower_priority", "lower_vaccine", "runtime","contact_reduction"))
...
summarise_age_outputs <- function(x) {
  mutate(x, 
         ...
         vaccine_1 = round(map_dbl(output_age, pull_total, outcome = "V1")),
         vaccine_2 = round(map_dbl(output_age, pull_total, outcome = "V2"))
 )
}
