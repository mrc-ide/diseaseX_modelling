...
test_both$population_size <- 10^6
...
tt_primary_doses <- c(0, 
                      ...
                      test_both$specific_vaccine_start + test_both$time_to_coverage_spec + test_both$specific_protection_delay)

second_dose_delay <- 1
protection_delay_rate <- 2

booster_doses <- c(0, daily_doses)
tt_booster_doses <- c(0, test_both$specific_vaccine_start + test_both$specific_protection_delay)

ve_bpsv <- list(infection = 0, disease = 0)
